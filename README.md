...
> 给改代码的人 / Developers / 開発者 → [`ARCHITECTURE.md`](ARCHITECTURE.md)
...
- 有对话在跑时,旁边叠出**状态卡**(最多 3 张),每张显示:
  - **标题** = 你在那个对话里说的第一句话。
  - **状态行**:思考/需确认时 = 当前状态 + 你最近的输入(若那句是 Claude 说的,会标 **`Claude:`** 区分);完成后 = 模型名 · 已完成。**若该会话回合结束了、但还有后台任务在跑(子代理 / 后台命令),卡片会显示「后台运行中」而不是假的「已完成」,并直接写出在跑什么**(如 `bg: 启动后端`)。带颜色和图标:
    - 🔵 正在思考(转圈)・ 🔵 后台运行中(回合完了但后台还有活,转圈)・ 🟡 需要你确认/选择(**!**,自动置顶)・ 🟢 已完成(**✓**)・ ⚪ 空闲・ ⚫ 已中断(Esc 打断了当前回合)
  - **悬停任意卡片**:弹出浮层,把被省略号截断的内容**完整展开**;若是「后台运行中」,还会**逐条列出在跑的所有后台任务**。
...
- When conversations are running, **status cards** stack next to it (up to 3). Each card shows:
  ...
  - **Status line**: while thinking / awaiting your input = state + your latest message (marked **`Claude:`** when that line is Claude's words, not yours); once done = model · Done. **If the turn is over but a background task is still running (a subagent or a background command), the card shows "Background running" instead of a misleading "Done" — and spells out what's running** (e.g. `bg: start backend`). With color and icon:
    - 🔵 Thinking (spinner) ・ 🔵 Background running (turn done, background still working — spinner) ・ 🟡 Needs your input (**!**, auto-floats to top) ・ 🟢 Done (**✓**) ・ ⚪ Idle ・ ⚫ Interrupted (you pressed Esc to stop the turn)
  - **Hover any card**: a tooltip pops up and shows the ellipsized content **in full**; for "Background running" it also **lists every background task** still going.
...
- **Cards look off?** (rare, on some GPUs) Create an empty `~/.claude/pet-data/card-region.flag` (or set `PET_CARD_REGION=1`) and restart the pet to fall back to the classic flat card.
...
- 会話が実行中だと、横に**状態カード**が重なって表示(最大3枚)。各カード:
  - **タイトル** = その会話で最初に入力した文。
  - **状態行**:考え中/確認待ちは 状態 + 最新の入力(その行が Claude の発言なら **`Claude:`** と表示);完了後は モデル名 · 完了。**ターンは終わったのに裏でタスク(サブエージェント / バックグラウンドコマンド)がまだ動いている場合は、誤解を招く「完了」ではなく「バックグラウンド実行中」を表示し、何が動いているかを直接示します**(例 `bg: バックエンド起動`)。色とアイコン付き:
    - 🔵 考え中(スピナー)・ 🔵 バックグラウンド実行中(ターン完了、裏でまだ稼働・スピナー)・ 🟡 確認/選択が必要(**!**、自動で最上部)・ 🟢 完了(**✓**)・ ⚪ 待機中・ ⚫ 中断済み(Esc でターンを中断)
  - **カードにホバー**:省略された内容を**全文**表示するツールチップが出ます。「バックグラウンド実行中」なら、動いている**すべてのバックグラウンドタスクを一覧**します。
...
- **カードの表示がおかしい?**(一部の環境)`~/.claude/pet-data` に空ファイル `card-region.flag` を作成(または `PET_CARD_REGION=1`)してペットを再起動すると、シンプルなカードに戻せます。
