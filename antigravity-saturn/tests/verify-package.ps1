...
Assert-True ($resident.Contains('Find-MarkedTerminalTarget')) 'Saturn does not search all Windows Terminal windows for its marked tab'
Assert-True ($resident.Contains('Saturn - Antigravity CLI')) 'Saturn terminal marker is missing'
Assert-True ($resident.Contains("ProcessName -ne 'WindowsTerminal'")) 'Saturn may still activate Windows Terminal through an arbitrary MainWindowHandle'
